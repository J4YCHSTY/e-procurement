...
// Pilihan role datang dari backend (App\Models\User::ROLES) supaya daftarnya
// cuma ditulis di satu tempat - kalau nanti ada peran baru, halaman ini ikut
// tanpa perlu diubah. Nilai di bawah ini cuma cadangan kalau propsnya kosong.
const ROLE_OPTIONS_FALLBACK = [
  { value: "user", label: "User" },
  { value: "head", label: "Kepala Departemen" },
  { value: "it_head", label: "Head of IT" },
...
function UserFormFields({ form, departements, entities, roles, roleLocked }) {
  const { data, setData, errors } = form;
  const roleOptions = roles?.length ? roles : ROLE_OPTIONS_FALLBACK;
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <div>
        ...
        >
          ...
          {entityIsLegacy && (
            <option value={data.entity}>{data.entity} (di luar daftar)</option>
          )}
        </select>
      ...
      <div>
        ...
        {roleLocked && (
          <p className="mt-1.5 text-xs text-ink-faint">
            Kamu tidak bisa mengubah role akun kamu sendiri. Minta rekan yang
            lain buat ubah ini.
          </p>
        )}
        ...
        <label className="flex items-start gap-3 rounded-[14px] border border-line bg-surface-sunken px-4 py-3.5">
          ...
          <span>
            ...
            <span className="mt-0.5 block text-[11.5px] leading-relaxed text-ink-muted">
              Terpisah dari Role di atas &ndash; centang ini kalau user perlu
              buka menu ini (kelola akun, reset password, dsb), lepas dari
              apakah dia ikut approve pengajuan atau tidak.
            </span>
          </span>
        </label>
        {roleLocked && (
          <p className="mt-1.5 text-xs text-ink-faint">
            Kamu tidak bisa mengubah akses Manajemen User akun kamu sendiri.
            Minta rekan yang lain buat ubah ini.
          </p>
        )}
...
export default function UsersIndex() {
  const { auth, flash, users, departements, entities, roles, filters, stats } =
    usePage().props;
            ...
            <tbody>
              {users.data.length === 0 && (
                <tr>
                  ...
                  >
                    Tidak ada akun yang cocok sama pencarian kamu.
                  </td>
                </tr>
              )}
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      {user.can_manage_users ? (
                        ...
                        <span className="text-xs text-ink-faint">&ndash;</span>
                      )}
                    </td>
                    <td className="px-4 py-3.5">
      ...
      <Modal show={showCreate} onClose={closeCreate}>
        <form onSubmit={submitCreate} className="p-7">
          ...
          <p className="mt-1 text-[12.5px] leading-relaxed text-ink-muted">
            Akun baru akan dibuat dengan password default. User bisa ganti
            password sendiri setelah login.
          </p>
      ...
      >
        <div className="p-7">
          ...
          <p className="mt-2 text-[13px] leading-relaxed text-ink-muted">
            Password <strong className="text-ink">{resettingUser?.name}</strong>{" "}
            akan dikembalikan ke password default. Beri tahu user yang
            bersangkutan supaya bisa login kembali lalu ganti passwordnya
            sendiri.
          </p>
      ...
      >
        <div className="p-7">
          ...
          <p className="mt-2 text-[13px] leading-relaxed text-ink-muted">
            {togglingUser?.is_active ? (
              <>
                <strong className="text-ink">{togglingUser?.name}</strong> tidak
                akan bisa login lagi sampai akunnya diaktifkan kembali. Riwayat
                pengajuan &amp; approval yang sudah ada tetap tersimpan.
              </>
            ) : (
              <>
                <strong className="text-ink">{togglingUser?.name}</strong> akan
                bisa login kembali seperti biasa.
              </>
            )}
          </p>
